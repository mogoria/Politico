...
class Party(Model):
    ...
    def get_all_parties(cls):
        parties = cls.select_all(table_name=cls.table_name, columns=cls.columns)
        return parties

    @classmethod
    def get_party_by_name(cls, name):
        party = cls.select_one(table_name=cls.table_name, criteria="name='{}'".format(name))
        return party

    @classmethod
    def get_party_id_from_name(cls, name):
        party_id = cls.select_one(table_name=cls.table_name, columns=['id'], criteria="name='{}'".format(name))
