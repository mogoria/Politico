...
class TestCandidateModel(BaseTestModel):
    ...
    def create_candidate(self, user_data, party_data, office_data):
        ...
        candidate_data = dict(email=user_data.get('email'),
    ...
    def test_add_candidate(self):
        ...
        new_candidate = self.create_candidate(self.user_data,
                                              self.party_data,
                                              self.office_data)
        ...
        cand = dict(candidate=user_id, party=party_id, office=off_id)

        self.assertEqual(new_candidate, cand)

    def test_get_all_candidates(self):
        # create two candidates
        self.create_candidate(self.user_data, self.party_data,
                              self.office_data)
        self.create_candidate(self.user_data2, self.party_data2,
                              self.office_data2)
    ...
    def test_get_candidate_by_email(self):
        new_cand = self.create_candidate(self.user_data, self.party_data,
                                         self.office_data)
        reteived_candidate = Candidate.get_candidate_by_email(
